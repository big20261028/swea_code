...
def bfs(st_pos, l, N):
    heap = deque([[st_pos[0], st_pos[1], 0]])
    visited = [[False] * N for _ in range(N)]
    visited[st_pos[0]][st_pos[1]] = True
    in_area_taxi = []
    if matrix[st_pos[0]][st_pos[1]] != 0:
        return matrix[st_pos[0]][st_pos[1]], (st_pos[0], st_pos[1])
    last_distance = 0
    while heap:
        x, y, dts = heap.popleft()
        if dts > l:
            break
        ...
        for dx, dy in dxy:
            ...
            visited[nx][ny] = True
            if matrix[nx][ny] != 0:
                in_area_taxi.append((matrix[nx][ny], (nx, ny)))
            heap.append([nx, ny, dts + 1])

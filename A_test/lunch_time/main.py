...
def cal_down_time(peoples,stair):
    ...
    stair_time_a = []
    stair_len = matrix[stair[0]][stair[1]]
    ...
    for i in range(len(stair_time_a)):
        arrival_time = stair_time_a[i]
        if i <= 2:
            start_time = arrival_time + 1
            finish_time = start_time + stair_len
        else:
            prev_finish_time = stair_time_a[i - 3]
            start_time = max(arrival_time + 1, prev_finish_time)
            finish_time = start_time + stair_len
        stair_time_a[i] = finish_time
    return max(stair_time_a)
